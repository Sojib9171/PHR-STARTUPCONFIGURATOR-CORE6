                                ...
                                <div class="row">
                                    <div class="col-md-6 offset-md-3" v-if="optionsLoaded">
                                        <select class="form-select" v-model="inputField">
                                            <option value="" disabled selected hidden>{{ this.data.response }}</option>
...
import { getAbsenceRowID, getUserID } from '@/localStorageUtils';
...
export default ({
    ...
    methods: {
        ...
        async checkForDuplicateLeaveTypeShortLeave(response) {
            this.isLoading = true;
            await this.$http.get(`/checkForDuplicateLeaveTypeShortLeave?responseText=${encodeURIComponent(response)}&userId=${encodeURIComponent(getUserID())}`, {
                headers: {
                    accept: "*/*",
                    Authorization:
                        "Basic " +
        ...
        async checkForDuplicateLeaveTypeStatutoryLeave(response) {
            this.isLoading = true;
            await this.$http.get(`/checkForDuplicateLeaveTypeStatutoryLeave?responseText=${encodeURIComponent(response)}&userId=${encodeURIComponent(getUserID())}`, {
                headers: {
                    accept: "*/*",
                    Authorization:
                        "Basic " +
