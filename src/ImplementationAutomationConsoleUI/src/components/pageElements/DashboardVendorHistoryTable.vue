<template>
    <div>
        <button @click="showModal = true" type="button" id="historyBtn" class="btn btn-primary" data-bs-toggle="modal"
            data-bs-target="#dashboradVndrHistryModal" hidden>
        </button>
        <!-- Modal -->
        <div class="modal fade" id="dashboradVndrHistryModal" tabindex="-1" aria-labelledby="dashboradVndrHistryModalLabel"
            aria-hidden="true">
            <div class="modal-dialog modal-xl modal-dialog-centered">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="dashboradVndrHistryModalLabel">Admin Approval History - {{
                            subsectionName }}</h5>
...
export default {
    ...
    methods: {
        ...
        loadItems() {
            this.$http.get(`/getVendorDashboardHistory?serverParams=${encodeURIComponent(JSON.stringify(this.serverParams))}`, {
                headers: {
                    accept: "*/*",
                    Authorization:
                        "Basic " +
