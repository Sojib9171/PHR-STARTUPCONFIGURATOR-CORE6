...
import { getDate, getUserID, saveCommonConfigRowID, getCommonConfigRowID, saveCommonConfigLogoImageName, saveCommonConfigMobileLogoImageName } from '@/localStorageUtils'
    ...
    methods: {
        ...
        async continueProcessForSelectOption() {
            ...
            if (this.data.question_no == 1) {
                if (getCommonConfigRowID() == null || getCommonConfigRowID() == '') {
                    await this.insertFirstRecord(getDate(), getUserID(), this.selectedRadio);
                }
                else {
        ...
        async insertFirstRecord(date, userId, response) {
            ...
            await this.$http
                ...
                .then((resp) => {
                    saveCommonConfigRowID(resp.data);
                    extendCookieTimeout();
        ...
        async updateRecord(response) {
            ...
            await this.$http
                ...
                .then(() => {
                    if (this.data.question_no == 2) {
                        saveCommonConfigLogoImageName(this.selectedFile.name);
                    }
                    else if (this.data.question_no == 4) {
                        saveCommonConfigMobileLogoImageName(this.selectedFile.name);
                    }
                    extendCookieTimeout();
                    this.isLoading = false;
                    this.$emit('next');
