<template>
  ...
  <button class="btn btn-logout" id="accessDeniedButton" @click="this.$router.push({ name: 'access-denied' });" hidden>
    <i class="ri-logout-circle-r-line"></i>
  </button>
</template>
...
export default {
  async mounted() {
    await this.emitter.on("accessDenied", (istrue) => {
      if (istrue == true) {
        $('#accessDeniedButton').click();
      }
    });
  },
