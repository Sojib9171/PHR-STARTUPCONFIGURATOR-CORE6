      ...
      <div class="page-body">
        <div>
          <div class="page-title">
            <!--Page title-->
            <h2>Dashboard for {{ moduleName }}</h2>
            <!--Breadcrumb-->
            <nav style="--bs-breadcrumb-divider: '|';" aria-label="breadcrumb">
              <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item"><router-link
                    :to="{ name: 'dashboard', params: { moduleName: 'EIM' } }"></router-link>
                  Dashboard</li>
              </ol>
            </nav>
          </div>
        </div>
        <section class="mt-3" v-if="this.moduleName != ''">
          <div class="section-body">
            <div class="config-block">
              <dashboardTable :key="componentKey" :moduleName="moduleName" />
            </div>
          </div>
        </section>
      </div>
...
import DashboardTable from "../components/pageElements/DashboardTable.vue";
...
export default {
  ...
  beforeRouteUpdate(to, from, next) {
    if (to.params.moduleName != from.params.moduleName) {
      this.moduleName = to.params.moduleName;
      this.componentKey += 1;
    }
    this.isLoading = true;
    setTimeout(() => {
