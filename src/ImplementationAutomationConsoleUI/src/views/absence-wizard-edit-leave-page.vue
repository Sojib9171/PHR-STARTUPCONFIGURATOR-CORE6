          ...
          <section class="mt-3">
            ...
            <div class="section-body">
              <div v-if="questionsLoaded">
                <absenceWizardFinalEdit :key="componentKey" :subsectionName="subsectionName" :recordId="rowId"
                  :totalQuestionCount="totalQuestionCount" :data="currentObject" @next="next" @back="back">
                </absenceWizardFinalEdit>
              </div>
            </div>
          </section>
        </div>
...
import AbsenceWizardFinalEdit from '../components/pageElements/AbsenceWizardFinalEdit.vue'
//import PageLoader from '@/components/pageElements/PageLoader.vue';
import { Base64 } from "js-base64";
import '../../css/styles.css';
//import { getAbsenceSubsectionName } from '@/localStorageUtils';
...
export default {
  ...
  methods: {
    ...
    next() {
      if (this.currentIndex < this.objects.length - 1) {
        ...
        this.$router.push({ name: 'absence-wizard-options', params: { subsection: this.subsectionName, id: this.rowId } });
      }
  ...
  components: {
    "sideBar": SideBar,
    "topBar": TopBar,
    "absenceWizardFinalEdit": AbsenceWizardFinalEdit
  },
  ...
  async mounted() {
    ...
    this.$http.get(`/getAbsenceWizardSelectedOptionsSummary?rowId=${encodeURIComponent(this.rowId)}&subsectionName=${encodeURIComponent(this.subsectionName)}`, {
      headers: {
        accept: "*/*",
        Authorization:
          "Basic " +
