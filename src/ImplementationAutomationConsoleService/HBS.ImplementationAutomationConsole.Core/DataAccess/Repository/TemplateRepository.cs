...
namespace HBS.ImplementationAutomationConsole.Core.DataAccess.Repository
{
    public class TemplateRepository : ITemplateRepository
    {
        ...
        public TemplateRepository(IBaseDataAccess iBaseDataAccess)
        {
        ...
        public async Task<List<string>> GetLeaveTypeCodesAsList()
        {
            string query = @"EXEC [SP_IA_GET_LEAVE_CODES]";
            ...
            using (var reader = await _iBaseDataAccess.ExecuteReaderAsync(query))
            {
                while (reader.Read())
                {
                    leaveTypeCodes.Add(reader.GetString(0));
                }
            }
