CREATE OR ALTER PROCEDURE [QADBV9_L2].[SP_IA_GET_DASHBOARD_HISTORY]
...
    DECLARE @RECORD_COUNT INT;
	SELECT b.MODULE_ID, c.SUBSECTION_ID, MODULE_NAME, SUBSECTION_NAME, APPROVAL_STATUS, APPROVAL_BY, APPROVAL_DATE,APPROVAL_COMMENT,APPROVAL_DATA,SIGNOFF_DATA INTO #temp1 FROM HS_HR_IA_DASHBOARD_INFO a LEFT JOIN HS_MODULE b ON a.MODULE_ID=b.MODULE_ID LEFT JOIN HS_HR_IA_SUBSECTIONS c ON a.SUBSECTION_ID=c.SUBSECTION_ID;

	SELECT * INTO #temp2
	FROM (
	 SELECT *
     , ROW_NUMBER() OVER (PARTITION BY SUBSECTION_NAME ORDER BY APPROVAL_DATE DESC) as rn
		FROM #temp1) t WHERE t.SUBSECTION_NAME='''+@SUBSECTION_NAME+''' ORDER BY t.APPROVAL_DATE DESC OFFSET 1 ROWS;

	SELECT * INTO #temp3
	FROM #temp2 WHERE 1=1 '+@WHERE_CLS+';

   	SELECT APPROVAL_DATE, APPROVAL_STATUS, APPROVAL_BY,APPROVAL_COMMENT,APPROVAL_DATA,SIGNOFF_DATA into #temp4
    FROM (
        SELECT *,
