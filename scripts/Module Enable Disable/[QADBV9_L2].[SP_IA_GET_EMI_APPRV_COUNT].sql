CREATE OR ALTER PROCEDURE [QADBV9_L2].[SP_IA_GET_EMI_APPRV_COUNT]
AS
BEGIN
	SELECT
		[RECORD_ID],
	...
	If ((SELECT COUNT(*) FROM HS_HR_IA_SUBSECTIONS WHERE MODULE_ID=2 AND IS_ENABLE=1)=0)
	BEGIN
	...
	BEGIN	
		...
		FROM #temp1 WHERE MODULE_ID=2 AND APPROVAL_STATUS='APPROVED' AND IS_ENABLE=1 AND(VENDOR_APPROVAL_STATUS='APPROVED' OR VENDOR_APPROVAL_STATUS IS NULL)) / (SELECT COUNT(*) FROM HS_HR_IA_SUBSECTIONS WHERE MODULE_ID=2 AND IS_ENABLE=1)
	END
