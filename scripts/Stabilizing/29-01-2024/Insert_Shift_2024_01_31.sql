...
SG.SHIFT_NAME, SG.SHIFT_ABR, SG.LEAVE_DAYS, (CASE WHEN SG.FLEXI_SHIFT = 1 THEN 0 ELSE SG.NEXT_DAY_SHIFT_OUT_TIME END), (CASE WHEN SG.FLEXI_SHIFT IS NULL THEN 0 ELSE SG.FLEXI_SHIFT END), 0, (CASE WHEN SG.CONTINUE_SHIFT IS NULL THEN 0 ELSE SG.CONTINUE_SHIFT END), 1, (CASE WHEN SG.OFF_SHIFT IS NULL THEN 0 ELSE SG.OFF_SHIFT END),
...
INSERT INTO [HS_TA_SHIFTDEF_DAY_CONFIG]
		   ...
		   ,[SFT_FLEXIHRS])
SELECT RIGHT('000000'+CAST(@SFTCodeCal + ROW_NUMBER() OVER (ORDER BY [ID]) AS varchar(25)),6), 
0, (CASE WHEN SG.FLEXI_SHIFT = 1 THEN 0 ELSE SG.NEXT_DAY_SHIFT_OUT_TIME END), (CASE WHEN SG.FLEXI_SHIFT IS NULL THEN 0 ELSE SG.FLEXI_SHIFT END), (CASE WHEN SG.CONTINUE_SHIFT IS NULL THEN 0 ELSE SG.CONTINUE_SHIFT END), (CASE WHEN SG.OFF_SHIFT IS NULL THEN 0 ELSE SG.OFF_SHIFT END), '#3f6b98', 1, 
