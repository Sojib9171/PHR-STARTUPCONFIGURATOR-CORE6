CREATE OR ALTER PROCEDURE [QADBV9_L2].[SP_IA_GET_SHRT_LEA_PNDNG_ITEMS]
@PAGE_INDEX INT,
@PAGE_SIZE INT,
...
    SET @WHERE_CLS = N'';
	...
	IF(@SEARCH_TEXT <> '')      
    BEGIN       
        SET @WHERE_CLS = @WHERE_CLS + N'AND SHORT_LEAVE_TYPE_CODE LIKE ''%' + @SEARCH_TEXT + N'%''';      
    END

    SELECT ID,SHORT_LEAVE_TYPE_CODE into #temp1
...
    DECLARE @RECORD_COUNT INT;
	...
	FROM #temp1 WHERE 1=1 '+@WHERE_CLS+';
   	...
   	SELECT ID,SHORT_LEAVE_TYPE_CODE into #temp3
    FROM (
        SELECT *,
